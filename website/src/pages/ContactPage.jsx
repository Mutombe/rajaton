...
export default function ContactPage() {
  ...
  const inputCls = "w-full bg-fg/5 border border-border-2 rounded-xl px-5 py-3.5 text-fg text-sm placeholder-fg-3 focus:outline-none focus:border-[#DC2626]/40 focus:ring-2 focus:ring-[#DC2626]/10 transition-all font-body";
      ...
      <section className="relative min-h-[65vh] flex items-center overflow-hidden mesh-hero noise">
        ...
        <div className="ctn py-20 md:py-40 relative z-10">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <p className="text-fg-4 text-base md:text-lg mt-8 max-w-lg">Whether you're a brand looking to scale or a retailer seeking solutions — we'd love to hear from you.</p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-5 gap-10 lg:gap-14">
            ...
            <Reveal className="lg:col-span-3" dir="left">
              <div className="glass-card rounded-[2rem] p-8 md:p-10">
                <h2 className="text-2xl font-bold text-fg font-heading mb-1.5">Send us a message</h2>
                <p className="text-fg-3 text-sm mb-8">We'll get back to you within 24 hours.</p>
                <form onSubmit={submit} className="space-y-5">
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-fg-4 text-xs mb-1.5 font-heading font-semibold">Full Name *</label>
                      <input type="text" value={form.name} onChange={e => up("name", e.target.value)} className={inputCls} placeholder="John Doe" required />
                    </div>
                    <div>
                      <label className="block text-fg-4 text-xs mb-1.5 font-heading font-semibold">Work Email *</label>
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-fg-4 text-xs mb-1.5 font-heading font-semibold">Company</label>
                      <input type="text" value={form.company} onChange={e => up("company", e.target.value)} className={inputCls} placeholder="Company name" />
                    </div>
                    <div>
                      <label className="block text-fg-4 text-xs mb-1.5 font-heading font-semibold">Service Interest</label>
                      <select value={form.service} onChange={e => up("service", e.target.value)} className={`${inputCls} appearance-none cursor-pointer`}>
                        <option value="" className="bg-surface-2">Select a service</option>
                        {SERVICES.map(s => <option key={s.id} value={s.id} className="bg-surface-2">{s.title}</option>)}
                        <option value="other" className="bg-surface-2">Other</option>
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="block text-fg-4 text-xs mb-1.5 font-heading font-semibold">Message *</label>
            ...
            <Reveal className="lg:col-span-2" dir="right" delay={0.15}>
              <div className="space-y-4">
                <h2 className="text-2xl font-bold text-fg font-heading mb-4">Global Offices</h2>
                {offices.map((o, i) => (
                  <motion.div key={i} className="glass-card rounded-2xl p-5 group" whileHover={{ x: 4 }}>
                    <h3 className="text-fg font-bold font-heading text-sm flex items-center gap-2">
                      <MapPin size={13} className="text-[#DC2626]" />{o.city}
                    </h3>
                    <p className="text-fg-3 text-xs mt-1.5">{o.addr}</p>
                    <div className="flex items-center gap-4 mt-2.5">
                      <p className="text-fg-3 text-xs flex items-center gap-1"><Phone size={10} />{o.phone}</p>
                      <p className="text-fg-4 text-[10px] flex items-center gap-1"><Clock size={9} />{o.tz}</p>
                    </div>
                  </motion.div>
                ))}

                <div className="glass-card rounded-2xl p-5 mt-4">
                  <h3 className="text-fg font-bold font-heading text-sm mb-3">Quick Contact</h3>
                  <div className="space-y-2.5">
                    <a href="mailto:partnerships@rajaton.com" className="text-fg-3 text-xs flex items-center gap-2.5 hover:text-[#DC2626] transition-colors"><Mail size={12} className="text-[#DC2626]" />partnerships@rajaton.com</a>
                    <a href="mailto:careers@rajaton.com" className="text-fg-3 text-xs flex items-center gap-2.5 hover:text-[#DC2626] transition-colors"><Mail size={12} className="text-[#DC2626]" />careers@rajaton.com</a>
                    <p className="text-fg-3 text-xs flex items-center gap-2.5"><Phone size={12} className="text-[#DC2626]" />+44 20 7946 0958</p>
                    <p className="text-fg-3 text-xs flex items-center gap-2.5"><Clock size={12} className="text-[#DC2626]" />Mon–Fri 9:00–18:00 GMT</p>
                  </div>
                </div>
              </div>
            </Reveal>
